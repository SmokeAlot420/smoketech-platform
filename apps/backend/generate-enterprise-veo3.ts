...
import { FFmpegVideoStitcher } from './src/pipelines/ffmpegStitcher';
...
async function generateEnterpriseVeo3(): Promise<void> {
  ...
  console.log('Complete production pipeline: VEO3 → FFmpeg → Topaz');
  ...
  try {
    ...
    const stitcher = new FFmpegVideoStitcher({
      outputPath: './generated/enterprise-stitched'
    });

    const topazEnhancer = new TopazVEO3Enhancer({
      inputPath: './generated/enterprise-stitched',
      outputPath: './generated/enterprise-enhanced',
    ...
    for (let i = 0; i < enterpriseSpecs.length; i++) {
      ...
      try {
        ...
        if (result.success) {
          console.log(`✅ Enterprise generation complete: ${result.enhancedPath}`);
...
async function generateSingleEnterpriseVideo(
  ...
  try {
    ...
    const enhancementResult = await topazEnhancer.enhanceVideo(veo3VideoPath, {
      model: 'Proteus',
      targetResolution: '4K',
      denoising: true,
      sharpening: true,
    ...
    return {
      ...
      enhancedPath: enhancementResult.success ? enhancementResult.outputPath : undefined,
      duration: 8,
      metadata: {
